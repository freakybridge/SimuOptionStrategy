...
switch inv
    ...
    case EnumType.Interval.day
        [is_err, md] = obj.FetchDailyMd(symb, exc, ts_s, ts_e, 'fut_daily', {'trade_date', 'open', 'high', 'low', 'close', 'amount', 'vol', 'oi', 'pre_settle', 'settle'}, 'Fetching future [%s%s] daily market data');
        if (~isempty(md))
            md(:, 11) = 0;
        end
    otherwise
