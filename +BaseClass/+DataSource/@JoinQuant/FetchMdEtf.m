...
switch inv
    case EnumType.Interval.min1
        [is_err, md] = FetchMin(obj, symb, exc, '1m', ts_s, ts_e, cnt, {'date', 'open', 'high', 'low', 'close', 'volume', 'money'}, 'Fetching etf [%s.%s] minitue market data'); 
        
    case EnumType.Interval.min5
        [is_err, md] = FetchMin(obj, symb, exc, '5m', ts_s, ts_e, cnt, {'date', 'open', 'high', 'low', 'close', 'volume', 'money'}, 'Fetching etf [%s.%s] minitue market data'); 
    ...
    case EnumType.Interval.day
        ...
        if (~is_err)
            ...
            md = [];
            obj.DispErr(sprintf('Fetching etf [%s.%s] daily market data', symb, exc));
        end
