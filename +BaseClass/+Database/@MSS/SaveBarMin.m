...
%       首次加入
function ret = SaveBarMin(obj, asset, md)
% 确定库名 / 端口 / 表名
...
for i = 1 : length(steps)
    ...
    % 入库
    tmp = '(''%s'', %f, %i, %i, %f, %f, %f, %f, %f, %f, %f),';
    tmp = repmat(tmp, 1, size(md_in, 2));
    tmp(end) = ';';
    tmp = ['INSERT INTO [%s] ([TIMESTAMP], [DATENUM], [DATE], [TIME], [OPEN], [HIGH], [LOW], [LAST], [AMOUNT], [VOLUME], [OI]) VALUES', tmp];
