...
classdef MSS < BaseClass.Database.Database
    ...
    methods (Hidden)
        ...
        function ret = CreateTriggerOverviews(obj, db, tb)
            ...
            tb_ins = tb;
            sql = sprintf("CREATE TRIGGER [dbo].[TriggerOverview] ON [%s] FOR UPDATE, INSERT " ...
                + " AS" ...
                + " BEGIN" ...
                + "     DECLARE @s datetime;" ...
                + "     DECLARE @e datetime;" ...
                + "     DECLARE @cnt int;" ...
                + "     DELETE FROM ""%s"" WHERE SYMBOL = '%s';" ...
                + "     SELECT @s = MIN(DATETIME), @e = MAX(DATETIME), @cnt = COUNT(*) FROM [%s];" ...
                + "     INSERT [%s] ([SYMBOL], [TS_START], [TS_END], [COUNTS]) VALUES ('%s', @s, @e, @cnt);" ...
                + " END", ...
                tb_ins, tb_ov, tb_ins, tb_ins, tb_ov, tb_ins);
            res = exec(conn, sql);
