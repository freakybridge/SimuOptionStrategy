...
for i = 1 : length(steps)
    ...
    % 入库
    tmp = '(''%s'', %f, %i, %i, %f, %f, %f, %f, %f, %f, %f, %f, %f, %i, %i),';
    tmp = repmat(tmp, 1, size(md_in, 2));
    tmp(end) = ';';
    tmp = ['INSERT INTO [%s] ([TIMESTAMP], [DATENUM], [DATE], [TIME], [OPEN], [HIGH], [LOW], [LAST], [AMOUNT], [VOLUME], [OI], [PRE_SETTLE], [SETTLE], [REM_N], [REM_T]) VALUES', tmp];
